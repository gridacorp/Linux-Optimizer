...
# + Firewall sin problemas de navegación + PDF/Office asociados vía xdg-mime
# + Wine completo con i386 + RustDesk pre-login + Proton VPN
# + AppArmor sin romper Flatpak + Repositorios con arch=amd64
# + 100% automático, sin prompts, reversible con backup
# =============================================================================
...
if [[ ! "$ID_LIKE" =~ (debian|ubuntu) ]] && [[ ! "$ID" =~ (debian|ubuntu|linuxmint|pop|zorin) ]]; then
  echo "❌ Distribución no soportada. Compatible con Debian, Ubuntu, Linux Mint, Pop!_OS, Zorin OS"
  exit 1
fi
...
# ──────────────────────────────────────────────────────────────
# 2b. CORRECCIÓN DE REPOSITORIOS: arch=amd64 + LIMPIEZA DE CONTRIB
# ──────────────────────────────────────────────────────────────
echo ""
echo "🔧 [2b/18] Corrigiendo repositorios para evitar warnings..."
...
if echo "$APT_OUTPUT" | grep -q "^E:"; then
  ...
  echo "⚠️ Continuando, pero algunas instalaciones podrían fallar."
else
  echo "✅ Índices actualizados."
fi

# ──────────────────────────────────────────────────────────────
# 4. DETECCIÓN DE HARDWARE Y ENTORNO
# ──────────────────────────────────────────────────────────────
echo ""
echo "🔍 [4/18] Detectando hardware y entorno..."
...
DISK_TYPE="SSD"
[ -n "$DISK_NAME" ] && [ -f "/sys/block/$DISK_NAME/queue/rotational" ] && [ "$(cat /sys/block/$DISK_NAME/queue/rotational)" = "1" ] && DISK_TYPE="HDD"
...
# ──────────────────────────────────────────────────────────────
# 5. SELECCIÓN DE PERFIL POR MATRIZ
# ──────────────────────────────────────────────────────────────
echo ""
echo "📊 [5/18] Calculando perfil adaptativo..."
...
case "$PROFILE" in
  LOW_HDD)  echo "🔴 LOW_RAM + HDD"; SWAP=133; CACHE_PRESSURE=100; DIRTY_RATIO=5; SCHED="bfq"; ZRAM_MB=$((RAM_MB*50/100)); ZRAM_ALGO="lz4"; PRELOAD_CYCLE=5; PRELOAD_HALFLIFE=1; THP="madvise" ;;
  LOW_SSD)  echo "🔵 LOW_RAM + SSD"; SWAP=100; CACHE_PRESSURE=50; DIRTY_RATIO=10; SCHED="mq-deadline"; ZRAM_MB=$((RAM_MB*50/100)); ZRAM_ALGO="lz4"; PRELOAD_CYCLE=2; PRELOAD_HALFLIFE=2; THP="madvise" ;;
  MID_HDD)  echo "🟡 MID_RAM + HDD"; SWAP=60; CACHE_PRESSURE=30; DIRTY_RATIO=10; SCHED="bfq"; ZRAM_MB=2048; ZRAM_ALGO="lz4"; PRELOAD_CYCLE=2; PRELOAD_HALFLIFE=3; THP="always" ;;
  MID_SSD)  echo "🟢 MID_RAM + SSD"; SWAP=40; CACHE_PRESSURE=20; DIRTY_RATIO=20; SCHED="mq-deadline"; ZRAM_MB=2048; ZRAM_ALGO="lz4"; PRELOAD_CYCLE=1; PRELOAD_HALFLIFE=4; THP="always" ;;
  HIGH_HDD) echo "⚡ HIGH_RAM + HDD"; SWAP=10; CACHE_PRESSURE=10; DIRTY_RATIO=15; SCHED="bfq"; ZRAM_MB=0; ZRAM_ALGO="none"; PRELOAD_CYCLE=1; PRELOAD_HALFLIFE=5; THP="always" ;;
  HIGH_SSD) echo "🚀 HIGH_RAM + SSD"; SWAP=1; CACHE_PRESSURE=5; DIRTY_RATIO=30; SCHED="mq-deadline"; ZRAM_MB=0; ZRAM_ALGO="none"; PRELOAD_CYCLE=0; PRELOAD_HALFLIFE=0; THP="always" ;;
esac
...
# ──────────────────────────────────────────────────────────────
echo ""
echo "⚙️ [6/18] Configurando kernel y sysctl..."
echo performance | tee /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor 2>/dev/null || true
cat > /etc/sysctl.d/99-flytux.conf <<EOF
vm.swappiness=$SWAP; vm.vfs_cache_pressure=$CACHE_PRESSURE; vm.dirty_ratio=$DIRTY_RATIO
vm.dirty_background_ratio=$((DIRTY_RATIO/2)); vm.page-cluster=0; vm.min_free_kbytes=$((RAM_MB*2048/100))
...
net.ipv4.tcp_timestamps=0; net.ipv4.tcp_sack=1; net.ipv4.tcp_window_scaling=1
kernel.sched_latency_ns=3000000; kernel.sched_min_granularity_ns=300000
kernel.sched_wakeup_granularity_ns=500000; kernel.sched_migration_cost_ns=500000
EOF
...
ENABLE=yes; SIZE=$ZRAM_MB; ALGO=$ZRAM_ALGO; PRIORITY=5
EOF
...
else systemctl disable preload 2>/dev/null || true; fi

# ──────────────────────────────────────────────────────────────
# 8. I/O SCHEDULER + TRIM
# ──────────────────────────────────────────────────────────────
echo ""
echo "💿 [8/18] Configurando I/O scheduler..."
[ -n "$DISK_NAME" ] && {
...
wget -q -O /tmp/oo.deb "$ONLYOFFICE_DEB" 2>/dev/null && {
  dpkg -i /tmp/oo.deb 2>/dev/null || true; apt install -f -y >/dev/null 2>&1 || true; rm -f /tmp/oo.deb
...
[ -n "$RUSTDEB_URL" ] && wget -q -O /tmp/rustdesk.deb "$RUSTDEB_URL" 2>/dev/null && dpkg -i /tmp/rustdesk.deb 2>/dev/null || apt install -y rustdesk 2>/dev/null || true
rm -f /tmp/rustdesk.deb 2>/dev/null
systemctl enable rustdesk.service 2>/dev/null || true; systemctl start rustdesk.service 2>/dev/null || true

echo "🔑 Instalando Proton VPN (no interactivo)..."
mkdir -p /usr/share/keyrings
curl -fsSL https://repo.protonvpn.com/debian/public_key.asc | gpg --dearmor --batch --yes -o /usr/share/keyrings/protonvpn-archive-keyring.gpg 2>/dev/null || true
...
apt update >/dev/null 2>&1 || true; apt install -y protonvpn 2>/dev/null || echo "   ⚠️ ProtonVPN: instalar manualmente"
echo "✅ Apps esenciales instaladas."

# ──────────────────────────────────────────────────────────────
# 13. ASOCIACIÓN MIME CON xdg-mime
# ──────────────────────────────────────────────────────────────
echo ""
echo "⚙️ [13/18] Configurando asociaciones MIME..."
...
set_defaults_as_user() {
  local USER="$1" UID=$(id -u "$1") DBUS="unix:path=/run/user/$UID/bus"
  mkdir -p "/home/$USER/.config"
  for MIME in application/pdf application/vnd.openxmlformats-officedocument.wordprocessingml.document application/vnd.openxmlformats-officedocument.spreadsheetml.sheet application/vnd.openxmlformats-officedocument.presentationml.presentation application/msword application/vnd.ms-excel application/vnd.ms-powerpoint application/rtf text/plain; do
...
mkdir -p /etc/skel/.config; cp /usr/share/applications/jopdf.desktop /etc/skel/.config/ 2>/dev/null || true
echo "✅ Asociaciones MIME aplicadas."

# ──────────────────────────────────────────────────────────────
# 14. FIREWALL HARDENED
# ──────────────────────────────────────────────────────────────
echo ""
echo "🛡️ [14/18] Configurando firewall..."
ufw --force reset >/dev/null 2>&1
ufw default deny incoming; ufw default allow outgoing
for PORT in 21 22 23 135 136 137 138 139 445 3389 5900; do ufw deny $PORT/tcp comment "Puerto $PORT bloqueado"; done
ufw allow out 53/tcp 2>/dev/null || true; ufw allow out 53/udp 2>/dev/null || true
ufw allow out 80/tcp 2>/dev/null || true; ufw allow out 443/tcp 2>/dev/null || true
...
ufw --force enable >/dev/null 2>&1; systemctl enable ufw 2>/dev/null || true
